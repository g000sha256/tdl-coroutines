...
private val FILES_DIRECTORY = createTempDirectory() // TODO
...
internal fun main() {
    runBlocking {
        subscribeToAuthorizationStateUpdates(coroutineScope = this)
...
private suspend fun getVersionOption() {
    ...
    when (val result = tdlClient.getOption(name = "version")) {
        is TdlResult.Failure -> logError(message = "[getVersionOption] Failed to get version (${result.code}, ${result.message})")
...
private suspend fun getGitCommitHashOption() {
    ...
    when (val result = tdlClient.getOption(name = "commit_hash")) {
        is TdlResult.Failure -> logError(message = "[getGitCommitHashOption] Failed to get git commit hash (${result.code}, ${result.message})")
        is TdlResult.Success -> {
            when (val optionValue = result.result) {
                is OptionValueString -> {
...
private suspend fun setTdlibParameters() {
    ...
    when (result) {
        is TdlResult.Failure -> logError(message = "[setTdlibParameters] Failure (${result.code}, ${result.message})")
