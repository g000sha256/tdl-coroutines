...
internal class TdlRepository(private val engine: TdlEngine) {
    ...
    init {
        updates = engine
            ...
            .transform { update -> transformUpdates(update = update) }
            .takeWhile { update -> !checkStopped(update = update) }
            .filter { update -> update !is ServiceUpdate }
    }

    @Suppress("UNCHECKED_CAST")
    suspend fun <F : Function, M> send(function: F): TdlResult<M> {
    ...
    private suspend fun FlowCollector<Update>.transformUpdates(update: Update) {
        emit(value = update)
