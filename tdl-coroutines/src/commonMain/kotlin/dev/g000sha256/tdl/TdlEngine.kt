...
internal class TdlEngine(
    ...
    private val responsesMutableSharedFlow = MutableSharedFlow<Triple<Int, Long, Any>>()
    private val updatesMutableSharedFlow = MutableSharedFlow<Triple<Int, Long, Any>>(extraBufferCapacity = Int.MAX_VALUE)
    ...
    fun getUpdates(clientId: Int): Flow<Update> {
        return updatesMutableSharedFlow.mapNotNull { triple ->
            val dto = triple.third
            if (triple.first == clientId && dto is Update) {
    ...
    suspend fun <F : Function> send(function: F, clientId: Int): Any {
        startIfNeeded()
        ...
        coroutineScope.launch(context = coroutineDispatcherReceiver) {
            while (true) {
                ...
                if (triple.second > 0L) {
                    ...
                    updatesMutableSharedFlow.emit(value = triple)
                }
            }
        }
    }
